...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("vizzotop"))
        return false;
...
{
    // Convert vizzotop:// to vizzotop:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("vizzotop://"))
    {
        uri.replace(0, 12, "vizzotop:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "vizzotop.lnk";
}
...
{
    return GetAutostartDir() / "vizzotop.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("vizzotop-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  vizzotop-qt [" + tr("command-line options") + "]                     " + "\n";

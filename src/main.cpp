...
const string strMessageMagic = "VizzoTop Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    {
        nSubsidy = 250000 * COIN;   // 250k Premine
    }
		...
		if(hasPayment && payeerewardpercent > 0 && payeerewardpercent < 100){
			...
			LogPrintf("Detected Masternode payment to %s\n", targetNode);	
					...
					foundPaymentAndPayee = true;
						...
						ExtractDestination(payee, address1);
						CBitcoinAddress address2(address1);
						if (pindexBest->nHeight+1 < 210000) {
						foundPaymentAmount = true;
						foundPayee = true;
						foundPaymentAndPayee = true;
						}
...
{
    int64_t ret = blockValue * 0.5; //50% for masternodes

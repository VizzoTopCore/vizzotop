...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: vizzotop.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: vizzotopd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizevizzotopamount=<n> " + _("Keep N vizzotop anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. vizzotop is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. vizzotop is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("vizzotop version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "VizzoTop server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of vizzotop") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart vizzotop to complete") << "\n";
    ...
    nAnonymizevizzotopAmount = GetArg("-anonymizevizzotopamount", 0);
    if(nAnonymizevizzotopAmount > 999999) nAnonymizevizzotopAmount = 999999;
    if(nAnonymizevizzotopAmount < 2) nAnonymizevizzotopAmount = 2;
    ...
    if(fEnableInstantX){
        ...
        if(nInstantXDepth < 0) nAnonymizevizzotopAmount = 0;
    } else {
        nInstantXDepth = 0;
    }
    ...
    LogPrintf("Anonymize vizzotop Amount %d\n", nAnonymizevizzotopAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1vizzotop+1000 == (.1vizzotop+100)*10
       10vizzotop+10000 == (1vizzotop+1000)*10
    */
